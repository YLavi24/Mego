...
def convert_to_custom_time(hour, minute, second, millisecond):
    custom_hour = hour
    total_seconds = hour * 3600 + minute * 60 + second + millisecond / 1000
    custom_part = (total_seconds % 3600) * 1080 / 3600
    custom_moment = ((total_seconds % 3600) * 1080 % 3600) / 3600 * 76
    return custom_hour, custom_part, custom_moment
...
<body>
    ...
    <script>
        ...
        function updateClock() {
            ...
            // Highlight current hour mark
            for (let i = 0; i < 24; i++) {
                const mark = document.getElementById(`hour-mark-${i}`);
                if (i === customHour) {
